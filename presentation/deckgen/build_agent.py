...
# ── 9-b. 리워크 에스컬레이션 ─────────────────────────────────
def s_rework():
    sl = slide('실패하면 — 한 칸에 하나씩만 올린다', 'FOLLOW-UP · 리워크')
    text(sl, 0.75, 1.9, 11.9, 0.4,
    ...
    text(sl, 1.05, 2.49, 5.5, 0.78, [
        ...
        {'t': '첫 실패에 곧바로 모델을 올리고 세션까지 새로 연다. '
              '싸게 끝날 일을 비싸게 만든다.',
    ...
    text(sl, 7.05, 2.49, 5.45, 0.78, [
        ...
        {'t': '축은 셋 — 재시도 · 범위 · 모델. 한 칸에 하나씩만 움직인다.',
         'sz': 12.5, 'c': MUTED, 'space_before': 4}])

    rows = [
        ('1회차', 'MID', '실패한 함수/블록만', '유지', '첫 배정'),
        ('2회차', 'MID', '실패한 함수/블록만', '유지',
         ('재시도 — 실패 로그·diff만 추가', ACC2)),
        ('3회차', 'MID', '파일 전체 + 호출부', ('새 세션', WARN),
         ('범위 \u2191  (모델은 그대로)', ACC2)),
        ('4회차', ('LARGE', WARN), '파일 전체 + 호출부', ('새 세션', WARN),
         ('모델 \u2191  (범위는 그대로)', WARN)),
        ('5회차', ('중단', RED), ('스펙으로 복귀', RED), ('새 세션', RED),
         ('구현을 더 시키지 않는다', RED)),
    ]
    table(sl, 0.75, 3.45, 11.85,
          ['시도', '티어', '다시 여는 범위', '세션', '이번에 올리는 축'],
          rows, colw=[1.15, 1.25, 3.6, 1.0, 4.85], sz=11.5, rowh=0.38)

    rect(sl, 0.75, 5.85, 11.85, 0.82, fill=CARD, radius=0.05)
    rect(sl, 0.75, 5.85, 0.055, 0.82, fill=WARN)
    text(sl, 1.05, 5.97, 11.4, 0.66, [
        {'t': '짧은 리워크에는 값을 물리지 않는다',
         'sz': 13.5, 'b': True, 'c': WARN},
        {'t': '1회 실패로 끝나면 사다리 비용은 제자리 재시도와 정확히 같다'
              '($0.0545). 리셋은 3회차부터라 맥락 재장전 입력도 그때 처음 붙는다.',
         'sz': 12, 'c': MUTED, 'space_before': 3}])

    note(sl, '세션 리셋은 실패 횟수가 아니라 방향이 어긋났을 때 건다 — '
              '검증 실패는 결함이고, 결함은 같은 맥락 안에서 고치는 편이 싸다.', y=6.78)
...
def s_rework_cost():
    ...
    rows = [
        ...
        ('1회', '$0.0545', '$0.0385', ('0.71배', ACC)),
        ('2회', '$0.0818', '$0.0519', ('0.63배', ACC)),
        ('3회', '$0.1090', '$0.1106', ('1.01배 \u2190 최악', WARN)),
        ('4회', '$0.1363', '$0.1106', ('0.81배', ACC)),
        ('10회', '$0.2998', '$0.1106', ('0.37배', ACC)),
        ('48회 (exp09-C)', '$1.3353', '$0.1106', ('0.08배', ACC)),
    ]
    table(sl, 0.75, 2.45, 7.5,
          ['실패 횟수', '제자리 재시도', '사다리', '배수'],
          rows, colw=[2.1, 1.85, 1.85, 1.7], sz=11.5, rowh=0.34)

    card(sl, 8.55, 2.45, 4.05, 1.28, '\u2460 리워크 턴은 짧게', [
        '진단+패치 900토큰 상한.',
    ], accent=ACC)
    ...
    text(sl, 1.05, 5.42, 11.4, 1.2, [
        {'t': '\u2462 공정하게 비교하면 — 출력 상한은 제자리 재시도에도 걸 수 있다',
         'sz': 13, 'b': True, 'c': ACC2},
        {'t': '48회 기준   제자리 $1.3353  \u2192  제자리+출력상한 $0.5673  '
              '\u2192  사다리 $0.1106',
         'sz': 12, 'c': MUTED, 'space_before': 4},
        {'t': '난제 시나리오 기대비용   제자리 $0.0661 (미해결 7.1%)  vs  '
              '사다리 $0.0475 (미해결 0.7%)',
         'sz': 12, 'c': ACC, 'space_before': 2},
        {'t': '절감의 대부분은 \u201c출력을 짧게\u201d에서 나오고, 사다리가 '
              '따로 기여하는 것은 멈출 지점을 정해 꼬리를 자르는 일이다.',
