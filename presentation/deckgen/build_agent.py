...
# ── 9-b. 리워크 에스컬레이션 ─────────────────────────────────
def s_rework():
    sl = slide('실패하면 — 모델만 올리지 않는다', 'FOLLOW-UP · 리워크')
    text(sl, 0.75, 1.9, 11.9, 0.4,
         [{'t': '라우팅은 첫 배정만 정한다. 돈이 실제로 타는 곳은 '
               '실패하고 다시 시작하는 구간이다.', 'sz': 14.5, 'c': MUTED}])

    rect(sl, 0.75, 2.4, 5.9, 1.0, fill=CARD, radius=0.06)
    rect(sl, 0.75, 2.4, 0.055, 1.0, fill=RED)
    text(sl, 1.05, 2.56, 5.5, 0.8, [
        {'t': '흔한 실패 (\u2717)', 'sz': 14, 'b': True, 'c': RED},
        {'t': '같은 범위에 더 비싼 모델을 다시 넣는다.',
         'sz': 12.5, 'c': MUTED, 'space_before': 4}])

    rect(sl, 6.75, 2.4, 5.85, 1.0, fill=CARD, radius=0.06)
    rect(sl, 6.75, 2.4, 0.055, 1.0, fill=ACC)
    text(sl, 7.05, 2.56, 5.45, 0.8, [
        {'t': '해야 할 일 (\u25cb)', 'sz': 14, 'b': True, 'c': ACC},
        {'t': '범위와 티어를 같이 올린다. 2회 이상 실패는 '
              '범위가 잘못 잘린 신호다.',
         'sz': 12.5, 'c': MUTED, 'space_before': 4}])

    rows = [
        ('1회차', 'MID', '실패한 함수/블록만', '첫 배정'),
        ('2회차', 'LARGE', '파일 전체 + 호출부', ('실패 로그·diff 첨부', ACC2)),
        ('3회차', 'LARGE', '모듈 + 인접 계약', ('테스트 먼저 작성 강제', ACC2)),
        ('4회차', ('중단', RED), ('스펙으로 복귀', RED),
         ('모델을 더 올리지 않는다', RED)),
    ]
    table(sl, 0.75, 3.62, 11.85,
          ['시도', '티어', '다시 여는 범위', '이번에 바뀌는 것'],
          rows, colw=[1.3, 1.7, 4.35, 4.5], sz=12.5, rowh=0.42)

    rect(sl, 0.75, 5.72, 11.85, 0.95, fill=CARD, radius=0.05)
    rect(sl, 0.75, 5.72, 0.055, 0.95, fill=WARN)
    text(sl, 1.05, 5.86, 11.4, 0.75, [
        {'t': '솔직히 — 이건 절감책이 아니라 보험이다',
         'sz': 13.5, 'b': True, 'c': WARN},
        {'t': '턴당 단가는 오른다(2회차 3.28배). 대신 4회차에서 구현을 '
              '멈추므로 총비용이 $0.21에서 상한이 걸린다. 제자리 재시도는 '
              '상한이 없다.',
         'sz': 12, 'c': MUTED, 'space_before': 3}])

    note(sl, '측정 지표는 비용이 아니라 같은 작업이 몇 번 돌아왔는가다 — '
              'exp09-C의 48턴 리워크(16.6배)가 그 경고다.', y=6.78)


# ── 9-c. 에스컬레이션의 비용 정직하게 ────────────────────────
def s_rework_cost():
    sl = slide('에스컬레이션은 비싸질까 — 숫자로', 'FOLLOW-UP · 리워크')
    text(sl, 0.75, 1.9, 11.9, 0.4,
         [{'t': '"비싼 모델로 올리면 비용이 폭발하지 않나" — 정당한 우려다. '
               '세 가지로 나눠 답한다.', 'sz': 14.5, 'c': MUTED}])

    rows = [
        ('0회 (성공)', '$0.0272', '$0.0272', ('동일', MUTED)),
        ('1회', '$0.0545', '$0.1166', ('2.14배', RED)),
        ('2회', '$0.0818', '$0.2132', ('2.61배 \u2190 최악', RED)),
        ('3회', '$0.1090', '$0.2132', ('1.96배', WARN)),
        ('10회', '$0.2998', '$0.2132', ('0.71배 \u2190 역전', ACC)),
        ('48회 (exp09-C)', '$1.3353', '$0.2132', ('0.16배', ACC)),
    ]
    table(sl, 0.75, 2.45, 7.5,
          ['실패 횟수', '제자리 재시도', '에스컬레이션', '배수'],
          rows, colw=[2.1, 1.85, 1.85, 1.7], sz=11.5, rowh=0.37)

    card(sl, 8.55, 2.45, 4.05, 1.45, '① 최악은 2.6배', [
        '실패 2회 구간이 가장 불리.',
        '절대값 $0.08 \u2192 $0.21 (13센트).',
    ], accent=RED)

    card(sl, 8.55, 4.02, 4.05, 1.9, '② 상한이 걸린다', [
        '4회차에 구현을 멈춘다.',
        '$0.2132에서 정지.',
        '제자리는 선형으로 계속 오른다.',
    ], accent=ACC)

    rect(sl, 0.75, 5.18, 7.5, 1.5, fill=CARD, radius=0.06)
    rect(sl, 0.75, 5.18, 0.055, 1.5, fill=ACC2)
    text(sl, 1.05, 5.31, 7.1, 1.32, [
        {'t': '\u2462 진짜 비용은 미해결 잔여다 (난제 시나리오, 4턴 소진 후)',
         'sz': 13, 'b': True, 'c': ACC2},
        {'t': '제자리 재시도  기대 $0.0804  \u2014  41.0% 확률로 여전히 미해결',
         'sz': 12, 'c': MUTED, 'space_before': 4},
        {'t': '에스컬레이션   기대 $0.1335  \u2014   1.4% 미해결, 스펙 문서가 남음',
         'sz': 12, 'c': ACC, 'space_before': 2},
        {'t': '개발자 1시간 $50 기준 41% \u00d7 1h = $20.5 기대손실. '
              '토큰 5센트를 아끼려다 $20를 태운다.',
         'sz': 12, 'c': WARN, 'space_before': 4}])

    note(sl, '결론 — 에스컬레이션은 평균 비용을 낮추는 장치가 아니라 '
              '꼬리 위험을 자르는 장치다. 절감은 라우팅이, 사고 방지는 이쪽이 한다.', y=6.85)
