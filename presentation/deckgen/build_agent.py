...
비싼 모델은 필요한 순간에만 불려 나온다.
    ...
    python3 presentation/deckgen/build_agent.py
"""
...
# ── 7. 구현 티어 ─────────────────────────────────────────────
def s_impl_tier():
    sl = slide('구현은 중간 모델로 내려도 되나', 'FOLLOW-UP · 구현 티어')
    text(sl, 0.75, 1.9, 11.9, 0.4,
         [{'t': '스펙이 정해진 구현은 상위 모델을 덜 탄다. 다만 무조건은 아니다.',
           'sz': 14.5, 'c': MUTED}])

    rect(sl, 0.75, 2.4, 5.85, 1.15, fill=CARD2, radius=0.05)
    rect(sl, 0.75, 2.4, 0.05, 1.15, fill=RED)
    text(sl, 1.05, 2.56, 5.4, 0.9, [
        {'t': '이렇게 내리면 사고가 난다 (✗)', 'sz': 13, 'b': True, 'c': RED},
        {'t': '"스펙만 정해졌으면 아무 모델이나 된다"',
    ...
    text(sl, 7.08, 2.56, 5.4, 0.9, [
        {'t': '내려도 되는 기준 (○)', 'sz': 13, 'b': True, 'c': ACC},
    ...
    text(sl, 1.05, 6.14, 11.4, 0.6, [
        {'t': '반대 방향 근거 — pass@1을 10% 올리려면 VRAM 약 4배가 필요하고, '
...
# ── 8. 배치 현실 ─────────────────────────────────────────────
def s_batch():
    sl = slide('배치 — 단가는 반값, 시간은 약속이 아니다', 'FOLLOW-UP · 비동기 처리')
    text(sl, 0.75, 1.9, 11.9, 0.4,
         [{'t': '반값은 사실이다. 문제는 언제 끝나는지를 아무도 약속하지 않는다는 것이다.',
           'sz': 14.5, 'c': MUTED}])
    ...
    card(sl, 7.45, 2.5, 5.15, 2.05, '실제로 깨진 사례', [
...
def s_guards():
    ...
    text(sl, 7.3, 2.6, 5.15, 2.0, [
        {'t': '밸브 2 — 세션 위생 (리셋은 마지막 수단)', 'sz': 16, 'b': True,
         'c': RED},
        {'t': '같은 설명이 반복되면 → 컴팩션. 세션은 살려두고 결론만 남긴다.',
         'sz': 12.5, 'c': MUTED, 'space_before': 6},
        {'t': '리셋은 완료된 결과가 원하던 방향이 아닐 때만 건다.',
         'sz': 12.5, 'c': ACC, 'space_before': 3},
        {'t': '검증 실패는 방향 문제가 아니다 → 리셋 말고 리워크.',
         'sz': 12.5, 'c': MUTED, 'space_before': 3},
        {'t': '반복을 곧바로 리셋으로 처리하면 아직 쓸모 있는 맥락까지 버린다.',
         'sz': 12, 'c': MUTED, 'space_before': 5}])

    bigstat(sl, 0.75, 4.95, 3.85, '−83%', '장황 342tok → 간결 58tok (exp05)', color=WARN)
    bigstat(sl, 4.78, 4.95, 3.85, '3회', '동일 설명 반복 시 컴팩션 (리셋 아님)',
            color=RED)
    bigstat(sl, 8.81, 4.95, 3.79, '16.6배', '48턴 리워크의 비용 (exp09-C)', color=ACC2)
    note(sl, '재생성한 출력은 다시 입력이 되어 이후 모든 턴의 단가를 올린다. '
              '그렇다고 세션을 통째로 버리면 맥락 재장전 비용을 또 문다 — 그래서 컴팩션이 먼저다.', y=6.75)


# ── 9-b. 리워크 에스컬레이션 (축소 사다리) ──────────────────
def s_rework():
    sl = slide('실패하면 — 칸은 세 개면 된다', 'FOLLOW-UP · 리워크')
    text(sl, 0.75, 1.9, 11.9, 0.4,
         [{'t': '라우팅은 첫 배정만 정한다. 돈이 실제로 타는 곳은 '
               '실패하고 다시 시작하는 구간이다.', 'sz': 14.5, 'c': MUTED}])

    rect(sl, 0.75, 2.35, 5.9, 0.95, fill=CARD, radius=0.06)
    rect(sl, 0.75, 2.35, 0.055, 0.95, fill=RED)
    text(sl, 1.05, 2.49, 5.5, 0.78, [
        {'t': '흔한 실패 (\u2717)', 'sz': 14, 'b': True, 'c': RED},
        {'t': '첫 실패에 곧바로 모델을 올린다. 칸을 잘게 쪼갤수록 '
              '안전해 보이지만 비싸지기만 한다.',
         'sz': 12.5, 'c': MUTED, 'space_before': 4}])

    rect(sl, 6.75, 2.35, 5.85, 0.95, fill=CARD, radius=0.06)
    rect(sl, 6.75, 2.35, 0.055, 0.95, fill=ACC)
    text(sl, 7.05, 2.49, 5.45, 0.78, [
        {'t': '해야 할 일 (\u25cb)', 'sz': 14, 'b': True, 'c': ACC},
        {'t': '범위를 넓히고 \u2192 안 되면 세션을 끊고 \u2192 안 되면 '
              '스펙으로 돌아간다. 세 칸.',
         'sz': 12.5, 'c': MUTED, 'space_before': 4}])

    rows = [
        ('1', 'widen', 'MID', '파일 전체 + 호출부', '유지',
         ('모델은 그대로 둔다', ACC2)),
        ('2', 'reset', 'MID', ('확정된 것만 이월', WARN), ('새 세션', WARN),
         ('입력 누적을 끊는다', ACC2)),
        ('3', 'respec', ('중단', RED), ('스펙으로 복귀', RED), ('새 세션', RED),
         ('구현을 더 시키지 않는다', RED)),
    ]
    table(sl, 0.75, 3.45, 11.85,
          ['칸', '이름', '티어', '다시 여는 범위', '세션', '이 칸이 하는 일'],
          rows, colw=[0.7, 1.35, 1.0, 3.5, 1.1, 4.2], sz=11.5, rowh=0.38)

    rect(sl, 0.75, 5.05, 11.85, 0.9, fill=CARD, radius=0.05)
    rect(sl, 0.75, 5.05, 0.055, 0.9, fill=ACC)
    text(sl, 1.05, 5.16, 11.4, 0.74, [
        {'t': '모델을 올리는 칸은 상시로 두지 않는다 — 조건부다',
         'sz': 13.5, 'b': True, 'c': ACC},
        {'t': '의도가 어긋나는 이유는 다섯 가지(범위·취향·전제·판단오류·맥락오염)인데, '
              '모델을 올려서 풀리는 건 그중 하나뿐이다. 분류기가 그 하나로 판정할 때만 칸을 끼운다.',
         'sz': 12, 'c': MUTED, 'space_before': 3}])

    rect(sl, 0.75, 6.05, 11.85, 0.62, fill=CARD2, radius=0.05)
    text(sl, 1.05, 6.14, 11.4, 0.48, [
        {'t': '칸을 일곱에서 셋으로 줄였더니 같은 대화 기록에서 '
              '$0.3574 \u2192 $0.1170 \u2014 3.06배 싸졌다.',
         'sz': 12.5, 'b': True, 'c': FG}])

    note(sl, '세션 리셋은 실패 횟수가 아니라 방향이 어긋났을 때 건다 — '
              '검증 실패는 결함이고, 결함은 같은 맥락 안에서 고치는 편이 싸다.', y=6.82)


# ── 9-c. 에스컬레이션의 비용 정직하게 ────────────────────────
def s_rework_cost():
    sl = slide('사다리는 비싸질까 — 숫자로', 'FOLLOW-UP · 리워크')
    text(sl, 0.75, 1.9, 11.9, 0.4,
         [{'t': '"단계를 밟다 보면 비용이 폭발하지 않나" — 정당한 우려다. '
               '측정해서 답한다.', 'sz': 14.5, 'c': MUTED}])

    rows = [
        ('0회 (성공)', '$0.0272', '$0.0272', ('동일', MUTED)),
        ('1회', '$0.0625', '$0.0465', ('1.34배', ACC)),
        ('2회', '$0.1073', '$0.0591', ('1.81배', ACC)),
        ('3회', '$0.1611', '$0.1141', ('1.41배', ACC)),
        ('4회', '$0.2239', '$0.1141', ('1.96배', ACC)),
        ('6회', '$0.3764', '$0.1141', ('3.30배', ACC)),
        ('10회', '$0.7895', '$0.1141', ('6.92배', ACC)),
    ]
    table(sl, 0.75, 2.45, 7.5,
          ['실패 횟수', '제자리 재시도', '사다리', '사다리가 싼 배수'],
          rows, colw=[2.0, 1.85, 1.75, 1.9], sz=11.5, rowh=0.34)

    card(sl, 8.55, 2.45, 4.05, 1.28, '\u2460 짧아도 이득이다', [
        '1회 26% \u00b7 2회 45% 절감.',
    ], accent=ACC)

    card(sl, 8.55, 3.9, 4.05, 1.28, '\u2461 상한이 걸린다', [
        '3칸에서 멈춰 $0.1141에서 정지.',
    ], accent=WARN)

    rect(sl, 0.75, 5.3, 11.85, 1.35, fill=CARD, radius=0.06)
    rect(sl, 0.75, 5.3, 0.055, 1.35, fill=ACC2)
    text(sl, 1.05, 5.42, 11.4, 1.2, [
        {'t': '\u2462 사다리가 싼 이유는 좋은 모델을 써서가 아니다 \u2014 입력이 안 불어나서다',
         'sz': 13, 'b': True, 'c': ACC2},
        {'t': '제자리에서 다시 시키면 그때까지의 대화가 통째로 입력에 다시 실린다. '
              '실측하면 턴마다 4,700토큰씩 불어난다.',
         'sz': 12, 'c': MUTED, 'space_before': 4},
        {'t': '이 누적을 빼고 계산하면   6회 실패 제자리 $0.1874  \u2192  '
              '실제로는 $0.3764. 절반으로 과소평가하게 된다.',
         'sz': 12, 'c': WARN, 'space_before': 2},
        {'t': '리셋 칸은 이 누적을 0으로 끊는다. 사다리의 이득은 거기서 나온다.',
         'sz': 11.5, 'c': MUTED, 'space_before': 4},
    ])


# ── 9-d. 반증 — 칸을 더 줄이면 어떻게 되나 ───────────────────
def s_rework_limit():
    sl = slide('그럼 두 칸으로 줄이면 더 싸지나', 'FOLLOW-UP · 리워크')
    text(sl, 0.75, 1.9, 11.9, 0.4,
         [{'t': '줄여서 싸졌으니 더 줄이면 더 싸질 것 같다. '
               '그 방향으로 끝까지 밀어보면 어디서 깨지는지 나온다.', 'sz': 14.5, 'c': MUTED}])

    rect(sl, 0.75, 2.4, 11.85, 0.78, fill=CARD, radius=0.05)
    rect(sl, 0.75, 2.4, 0.055, 0.78, fill=WARN)
    text(sl, 1.05, 2.5, 11.4, 0.62, [
        {'t': '모델을 올려야만 풀리는 일이 실제로 있다',
         'sz': 13.5, 'b': True, 'c': WARN},
        {'t': '모델 올리는 칸을 아예 없애면, 그런 일은 세 칸을 다 쓰고도 못 풀고 '
              '사람에게 넘어간다. 토큰은 아꼈는데 사람 시간을 쓴다.',
         'sz': 12, 'c': MUTED, 'space_before': 3}])

    rows = [
        ('조건부 칸 있음', 'widen \u2192 모델 \u2191 \u2014 해결',
         '$0.1377', ('\u2014', MUTED), ('$0.1377', ACC)),
        ('없앤 경우', 'widen \u2192 reset \u2192 respec \u2014 사람 호출',
         '$0.0832', '$0.10', ('$0.1832  (1.3\u00d7)', WARN)),
        ('', '', '', '$0.50', ('$0.5832  (4.2\u00d7)', RED)),
        ('', '', '', '$2.00', ('$2.0832  (15.1\u00d7)', RED)),
    ]
    table(sl, 0.75, 3.28, 11.85,
          ['설정', '경로', '토큰', '사람 시간', '실질 비용'],
          rows, colw=[2.3, 4.5, 1.35, 1.5, 2.2], sz=11.5, rowh=0.355)

    rect(sl, 0.75, 5.32, 5.9, 1.02, fill=CARD, radius=0.06)
    rect(sl, 0.75, 5.32, 0.055, 1.02, fill=ACC)
    text(sl, 1.05, 5.43, 5.5, 0.86, [
        {'t': '그래서 지우지 않고 조건부로 둔다', 'sz': 13, 'b': True, 'c': ACC},
        {'t': '상시 칸에서는 빼되, 분류기가 난이도 문제로 판정할 때만 '
              '한 칸 끼운다.', 'sz': 12, 'c': MUTED, 'space_before': 3}])

    rect(sl, 6.75, 5.32, 5.85, 1.02, fill=CARD, radius=0.06)
    rect(sl, 6.75, 5.32, 0.055, 1.02, fill=MUTED)
    text(sl, 7.05, 5.43, 5.45, 0.86, [
        {'t': '사람 시간은 가정값이다', 'sz': 13, 'b': True, 'c': FG},
        {'t': '$0.10 / $0.50 / $2.00 세 가지로 벌려 놓았다. '
              '어느 값을 넣어도 결론은 뒤집히지 않는다.',
         'sz': 12, 'c': MUTED, 'space_before': 3}])

    note(sl, '줄이는 게 목적이 아니라 값을 하는 칸만 남기는 게 목적이다 \u2014 '
              '값을 하는 칸까지 지우면 비용은 다른 데로 옮겨갈 뿐이다.', y=6.55)
...
def s_demo():
    ...
    for i, ln in enumerate(out):
        ...
        if ln.startswith('['):
            c = ACC2
        parts.append({'t': ln or ' ', 'f': 'Consolas', 'sz': 11.5,
                      'c': c, 'space_before': 0 if i == 0 else 1})
    text(sl, 1.02, 2.62, 7.4, 3.6, parts)
...
def s_closing():
    ...
    note(sl, '본편(22장) · 보너스(10장) · 후속(15장) — 각각 독립 발표 가능합니다.', y=7.02)
...
BUILD = [cover, s_problem, s_taxonomy, s_architecture, s_router_code,
         s_web_handoff, s_impl_tier, s_batch, s_guards, s_rework, s_rework_cost, s_rework_limit, s_demo,
