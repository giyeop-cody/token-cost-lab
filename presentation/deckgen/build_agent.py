...
def s_rework_cost():
    ...
    rows = [
        ...
        ('1회', '$0.0545', '$0.0545', ('동일 \u2190 차액 없음', ACC)),
        ('2회', '$0.0818', '$0.0839', ('1.03배', MUTED)),
        ('3회', '$0.1090', '$0.1763', ('1.62배 \u2190 최악', RED)),
        ('4회', '$0.1363', '$0.1763', ('1.29배', WARN)),
        ('10회', '$0.2998', '$0.1763', ('0.59배 \u2190 역전', ACC)),
        ('48회 (exp09-C)', '$1.3353', '$0.1763', ('0.13배', ACC)),
    ]
    table(sl, 0.75, 2.45, 7.5,
          ['실패 횟수', '제자리 재시도', '사다리', '배수'],
          rows, colw=[2.1, 1.85, 1.85, 1.7], sz=11.5, rowh=0.34)

    card(sl, 8.55, 2.45, 4.05, 1.28, '\u2460 짧으면 차액이 없다', [
        '1회 실패까지 비용이 같다.',
    ], accent=ACC)
    ...
    text(sl, 1.05, 5.42, 11.4, 1.2, [
        {'t': '\u2462 진짜 비용은 미해결 잔여다 (난제 시나리오, 5턴 소진 후)',
         'sz': 13, 'b': True, 'c': ACC2},
        {'t': '제자리 재시도  기대 $0.0756  \u2014  16.8% 확률로 여전히 미해결',
         'sz': 12, 'c': MUTED, 'space_before': 4},
        {'t': '사다리        기대 $0.0719  \u2014   0.4% 미해결, 스펙 문서가 남음',
         'sz': 12, 'c': ACC, 'space_before': 2},
        {'t': '축을 하나씩 올리면 비싼 칸에 닿기 전에 끝나는 경우가 많다 \u2014 '
              '그래서 더 싸면서 더 많이 해결한다. 남는 16.4%p는 개발자 1시간 '
              '$50 기준 $8.2의 기대손실이다.',
         'sz': 12, 'c': WARN, 'space_before': 4}])

    note(sl, '사다리의 값어치는 평균 비용을 낮추는 데 있지 않다 — '
              '꼬리 위험을 자르는 데 있다. 절감은 라우팅이, 사고 방지는 이쪽이 한다.', y=6.85)
