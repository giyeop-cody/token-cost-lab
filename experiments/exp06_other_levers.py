...
def main():
    ...
    report.table(
        ...
        [["할인", "입력·출력 50%", "입력·출력 50%"],
         ["SLA", "24시간 내", "24시간 내 (실제 지연은 실행 조건별 확인)"],
    ...
    print("  → 토큰 수는 그대로다. 지원 서비스에서 가격을 줄이는 레버다. 지연·실패·지원 조건은 확인해야 한다.")
    ...
    ctx = 20_000
    rows = []
    for ratio in (1,2,5,10,20,30):
        loss = "변경 없음" if ratio == 1 else "미측정 (작업별 평가 필요)"
        t = int(ctx / ratio)
        c = t * m.inp / 1e6
        rows.append([f"{ratio}x", f"{t:,}", pricing.usd(c),
                     f"{(1-1/ratio)*100:.0f}%", loss])
    report.table(["압축비", "입력tok", "요청당 입력비용", "절감", "정확도 변화 (자체 미측정)"],
                 rows, ["r", "r", "r", "r", "l"])
    print("  · LLMLingua/LongLLMLingua/LLMLingua-2는 외부 연구다. 모델·데이터·압축비별 원문 조건을 확인한다.")
    print("  · 위 비용은 압축 토큰 수 가정의 산술이며, 품질 결과를 임의의 보고치로 채우지 않는다.")
    report.table(
        ["콘텐츠 종류", "시도용 가정 (보장 아님)", "검증 조건"],
        [["지시·제약·수용조건", "80~90%", "지우면 요구사항이 바뀐다"],
         ["few-shot 예시", "40~60%", "패턴 손실과 정답률 확인"],
         ["검색된 RAG 컨텍스트", "20~40%", "질의별 중요 내용 누락 확인"],
         ["코드 구문·수식·ID", "의미·식별자 보존", "무손실 형식 정규화만 우선"]],
    )
    print("  ⚠️ 압축기를 돌리는 비용과 지연도 계산에 넣어야 한다.")
    print("     짧은 프롬프트는 압축 비용이 이득을 넘을 수 있다. 실제 손익을 확인한다.")
    print("  💡 위 절감률은 가정이다. 중복 제거·표 정규화의 실제 감소량과 품질을 별도로 확인한다.")
    ...
    print(f"  잔여비용에 순차 적용하는 가정의 산술 결과: {(1-remaining)*100:.3f}%. 60~80% 범위 밖이다.")
    print("  ⚠️ 검증된 상한도 아니다. 레버 중복·상호작용을 측정하지 않은 예시 시나리오다.")
    print("     반드시 자기 데이터로 A/B를 돌린 뒤 숫자를 확정할 것.")

    report.section("F. 적용 우선순위 — 노력 대비 효과")
    report.table(
        ["순위", "레버", "난이도", "리스크", "먼저 볼 조건"],
        [["1", "프롬프트 캐싱", "낮음", "쓰기·저장료", "모델별 최소 프리픽스 확인"],
         ["2", "verbosity / effort", "낮음", "낮음", "설정 한 줄로 끝나는 경우가 많음"],
         ["3", "Batch API", "낮음", "지연·지원조건", "지연에 둔감한 잡이 있는가"],
